...
def get_veff_mpi(ks, cell=None, dm=None, dm_last=0, vhf_last=0, hermi=1,
    ...
    if isinstance(kpts, numpy.ndarray):
        return krks.get_veff_mpi(ks, cell, dm, dm_last, vhf_last, hermi, kpts, kpts_band)
    ...
    if ks.grids.non0tab is None:
        _setup_becke_grids(ks, dm_bz, ground_state)
    ...
    if not hybrid:
        ...
        if ground_state:
            exc -= (numpy.einsum('K,Kij,Kji', weight, dm[0], vk[0]) +
                    numpy.einsum('K,Kij,Kji', weight, dm[1], vk[1])).real * .5

    if ground_state:
        ecoul = numpy.einsum('K,Kij,Kji', weight, dm[0]+dm[1], vj).real * .5
    else:
        ecoul = None
