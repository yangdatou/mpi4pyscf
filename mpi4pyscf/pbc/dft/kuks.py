...
def get_veff_mpi(ks, cell=None, dm=None, dm_last=0, vhf_last=0, hermi=1,
    ...
    if hermi == 2:  # because rho = 0
        ...
        n, exc, vxc = ni.nr_uks(cell, ks.grids, ks.xc, dm, 
                                      0, kpts, kpts_band, 
    ...
    if not hybrid:
        ...
        if ground_state:
            exc -= (numpy.einsum('Kij,Kji', dm[0], vk[0]) +
                    numpy.einsum('Kij,Kji', dm[1], vk[1])).real * .5 * weight

    if ground_state:
        ecoul = numpy.einsum('Kij,Kji', dm[0]+dm[1], vj).real * .5 * weight
    else:
        ecoul = None
...
def _setup_becke_grids(ks, dm, ground_state):
    ...
    if (isinstance(ks.grids, gen_grid.BeckeGrids) 
        ...
        rho = ni.get_rho(cell, dm, grids, ks.max_memory)
